...
class SP3:
    def __init__(self, filepath=None):
        ...
        if self.filepath and os.path.exists(self.filepath):
            ...
            warnings.warn(f"SP3 文件路径无效或不存在: {self.filepath}")
...
def gnss_eph_interpolate(df, sat_id, target_time):
    """
    对 GNSS 星历数据进行插值，返回指定卫星在目标时间的状态向量。
    这里假设 df 已经按照时间排序，并且包含 'sat_id' 列和时间列（如 'timesystem'）。
    """
    sat_df = df[df['sat_id'] == sat_id]
    if sat_df.empty:
        print(f"未找到卫星 {sat_id} 的星历数据！")
        return None, None
    
    times = sat_df['timesystem'].values
    times_seconds = np.array([(t - target_time) for t in times])
    coords = sat_df[['x', 'y', 'z']].values
    
    interpolated_coord, err = MathTool.window_interpolate(times, coords, target_time)
    
    return interpolated_coord, err