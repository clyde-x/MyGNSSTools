...
def write_merged_rinex(file_path, header, data):
    with open(file_path, 'w', encoding='utf-8') as file:
        ...
        for epoch, info in sorted(data.items()):
            updated_epoch_line = f"> {info['time']}  {info['flag']}{str(info['num_satellites']).rjust(3)}\n"
            file.write(updated_epoch_line)
            for sat_id in sorted(info['observations']):
                file.write(info['observations'][sat_id]+'\n')
...
if __name__ == '__main__':
    ...
    file_path2 = r"D:\csu\dataset\DROL-GNSS\Obs_pco\DL0100LEO_20243600_PCO.rnx"
    file_path1 = r"D:\csu\dataset\DROL-GNSS\Obs_pco\DL0200LEO_20243600_PCO.rnx"
