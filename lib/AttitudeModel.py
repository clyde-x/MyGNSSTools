...
def read_attitude_file(filepath):
    ...
    """
    # ATT-L1B files contain a descriptive header followed by ten numeric
    # columns.  Read only numeric quaternion rows so callers can use the
    # original ATT-L1B path directly instead of maintaining a second copy.
    rows = []
    with open(filepath, encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            try:
                ...
                continue
            rows.append(fields)
    tmpdata = pd.DataFrame(rows, columns=['year', 'month', 'day', 'hour', 'minute', 'second',
                                          'q1', 'q2', 'q3', 'q4'])
