...
if __name__ == "__main__":
    ...
    config_path = './configs/PCOConfig.json'
    config = json.load(open(config_path, 'r'))

    start_time = TimeSystem.TimeSystem().from_str(config['start_time'])
    end_time = TimeSystem.TimeSystem().from_str(config['end_time'])
    ...
    for antenna in config['antennas']:
        ...
        if valid_target_times:
            interpolated_quats = MathTool.batch_quaternion_interpolate(
                att_times_array, att_quat_array, np.array(valid_target_times)
            )
