...
def SppInfo(log_path, satellite, date):
  ...
  spp_info_df = pd.read_csv(spp_info_file, header=None, sep=r'\s+', skiprows=1, encoding='gbk')
...
def PlotLog(config):
  ...
  if config['rdod_res']:
    ...
    result = PlotTool.visualize_res_sky(rdod_res, 'RDOD', config)
    if config["if_log"]:
      log_file.write(result+'\n')
  if config['kipp_res']:
    kipp_res = ResLog(log_path, satellite, date, 'KIPP')
    result = PlotTool.visualize_res_sky(kipp_res, 'KIPP', config)
    if config["if_log"]:
      log_file.write(result+'\n')
...
if __name__ == "__main__":
  json_path = 'd:/csu/MyTools/configs/AnalysisConfig_DRO_template.json'
